...
# ------------------------------------------------------------
# Export to .scad with Hexagonal Grid
# ------------------------------------------------------------
def export_terrain_to_scad(height_map, hex_size, filename, gradient_factor):
    n = len(height_map)
    min_h, max_h = get_min_max_height(height_map)
    
    # Pre-calculate hexagon vertices 
    # Vertices for a regular hexagon with side length = hex_size
    hex_pts = [
        (hex_size, 0),
    ...
    # Format points string for OpenSCAD
    points_str = "[" + ", ".join(f"[{pt[0]:.3f}, {pt[1]:.3f}]" for pt in hex_pts) + "]"
    ...
    with open(filename, "w") as f:
        ...
        for i in range(n):
            for j in range(n):
                h = height_map[i][j]
                # Compute center for hexagon
                # Horizontal spacing: 1.5 * hex_size
                # Vertical spacing: hex_size * sqrt(3), with a half offset for odd columns
                center_x = i * (1.5 * hex_size)
                center_y = j * (hex_size * math.sqrt(3)) + (i % 2) * (hex_size * math.sqrt(3)/2)
                
                if h > 0:
                    z = 1
                    # Scale terrain height (-1 for the base)
                    height_extrude = h * hex_size - 1
                    ...
                    f.write(f"  translate([{center_x:.3f}, {center_y:.3f}, {z}])\n")
                    f.write("    linear_extrude(height=" + f"{height_extrude:.3f}" + ")\n")
                    f.write("      polygon(points=" + points_str + ");\n")
                
                # Always place a base hexagon with a height of 1 (blue color)
                f.write("  color(\"blue\")\n")
                f.write(f"  translate([{center_x:.3f}, {center_y:.3f}, 0])\n")
...
if __name__ == "__main__":
    ...
    island_radii = [N / 4, N / 2]  # Values for island sizes

    # Apply the island mask using the seeds
    ...
    export_terrain_to_scad(perlin_island, hex_size, output_file, gradient_factor)
